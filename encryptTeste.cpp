...
int main(int argc, const char * argv[])
{
	...
	uint32_t scaleModSize = 50;	// scale module
    	uint32_t batchSize = 1<<7;	// batch size or how many slots per pack
	...
	// todo: make this a function	
    	...
    	ifstream inputW1 (DATASETFOLDER + "/w1.txt");
	...
	cout << "W1 loaded w/ shape: (" << W1.size()<< ", " << W1[0].size() << ")\n";
    	...
    	ifstream inputW2 (DATASETFOLDER + "/w2.txt");
     	if(inputW2.is_open())
	{
    		string line;
    	 	vector<string> line_v;
	...
	cout << "W2 loaded w/ shape: (" << W2.size()<< ", " << W2[0].size() << ")\n";
    	...
    	ifstream inputW3 (DATASETFOLDER + "/w3.txt");
     	if(inputW3.is_open())
	{
    		string line;
    	 	vector<string> line_v;
